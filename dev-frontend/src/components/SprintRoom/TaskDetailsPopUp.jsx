  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm z-50 p-4">
      <div className="relative w-full max-w-6xl max-h-[90vh] overflow-hidden">
        <div className="bg-gradient-to-br from-[#1a1a1a] to-[#2d1b3d] text-white rounded-2xl shadow-2xl border border-[#8D2B7E]/20">
          {/* Header */}
          <div className="flex items-center justify-between p-6 border-b border-[#8D2B7E]/20">
            <div className="flex items-center gap-3">
              <div className="w-3 h-3 bg-[#8D2B7E] rounded-full"></div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                ...
                <div className="bg-black/40 p-4 rounded-xl border border-[#8D2B7E]/20">
                  <h3 className="text-lg font-semibold text-[#8D2B7E] mb-2">Task Title</h3>
                  <p className="text-white text-xl">{task.title}</p>
                </div>

                {/* Assigned Members */}
                <div className="bg-black/40 p-4 rounded-xl border border-[#8D2B7E]/20">
                  <h3 className="text-lg font-semibold text-[#8D2B7E] mb-3">Assigned Members</h3>
                  ...
                  <div className="space-y-2 mb-4">
                    {taskData.assignedMembers?.length > 0 ? (
                      taskData.assignedMembers.map((member) => (
                        <div
                          key={member._id}
                          className="flex items-center justify-between bg-gray-800/50 p-3 rounded-lg border border-gray-700/50"
                        >
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 bg-[#8D2B7E] rounded-full flex items-center justify-center text-sm font-semibold">
                  ...
                  {isSprintCreator && (
                    <div className="border-t border-gray-700/50 pt-4">
                      ...
                      <select
                        multiple
                        className="w-full p-3 bg-gray-800/50 border border-gray-700/50 rounded-lg text-white focus:border-[#8D2B7E] focus:outline-none"
                      ...
                      <button
                        ...
                        className="mt-3 px-4 py-2 bg-[#8D2B7E] text-white rounded-lg hover:bg-[#8D2B7E]/80 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                      >
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {isSprintCreator && (
                      <button
                        ...
                        className="text-sm text-[#8D2B7E] hover:text-[#8D2B7E]/80 transition-colors"
                      >
                  ...
                  {isEditing ? (
                    <div className="space-y-3">
                      <textarea
                        ...
                        className="w-full p-3 bg-gray-800/50 border border-gray-700/50 rounded-lg text-white focus:border-[#8D2B7E] focus:outline-none resize-none"
                        rows="4"
              ...
              <div className="bg-black/40 p-4 rounded-xl border border-[#8D2B7E]/20">
                <h3 className="text-lg font-semibold text-[#8D2B7E] mb-4">Comments</h3>
                ...
                <div className="space-y-3 mb-4 max-h-64 overflow-y-auto pr-2">
                  {taskData.comments?.length > 0 ? (
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-1">
                              <div className="w-6 h-6 bg-[#8D2B7E] rounded-full flex items-center justify-center text-xs font-semibold">
                    ...
                    <p className="text-gray-400 italic text-center py-4">No comments yet</p>
                  )}
                </div>
                ...
                <div className="border-t border-gray-700/50 pt-4">
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 p-3 bg-gray-800/50 border border-gray-700/50 rounded-lg text-white focus:border-[#8D2B7E] focus:outline-none"
                      disabled={loading}
                    />
                    <button
                      onClick={handleAddComment}
                      disabled={!newComment.trim() || loading}
                      className="px-4 py-3 bg-[#8D2B7E] text-white rounded-lg hover:bg-[#8D2B7E]/80 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                    >
                      {loading ? "..." : "Send"}
                    </button>
                  </div>
          ...
          {/* Error Message */}
          {error && (
            <div className="absolute top-4 left-4 right-4 bg-red-900/20 border border-red-500/50 text-red-300 p-3 rounded-lg">
              {error}
              <button
