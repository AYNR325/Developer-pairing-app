  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col lg:flex-row">
      ...
      <div className="flex-1 flex flex-col w-full lg:w-auto">
        {/* Header */}
        <div className="bg-[#2D033B] border-b border-[#8D2B7E]/20 p-3 sm:p-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
            <div className="flex items-center gap-3 w-full sm:w-auto">
              {/* Mobile/Tablet Hamburger */}
              <button
                onClick={() => setSidebarOpen(!sidebarOpen)}
                className="lg:hidden w-10 h-10 flex items-center justify-center text-white hover:bg-gray-800 rounded-lg transition-colors flex-shrink-0 z-50 relative"
              ...
              <div>
                <h1 className="text-xl sm:text-2xl font-bold">Sprint Chat</h1>
              ...
              <button
                ...
                className="ml-auto bg-[#8D2B7E] text-white px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg hover:bg-[#A259C6] transition-colors text-xs sm:text-sm md:ml-[29rem] lg:hidden"
              >
                Back
              </button>
            </div>
              ...
              <div className="flex gap-1">
                {sprintMembers
                  ...
                  .map(member => (
                    <div
                      key={member._id}
                      className="w-7 h-7 sm:w-8 sm:h-8 bg-[#8D2B7E] rounded-full flex items-center justify-center text-xs font-semibold relative"
                      title={member.username}
                    >
                      {member.username?.charAt(0)?.toUpperCase()}
                      <div className="absolute -bottom-1 -right-1 w-2.5 h-2.5 sm:w-3 sm:h-3 bg-green-500 rounded-full border-2 border-[#2D033B]"></div>
                    </div>
                  ))}
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
                      ...
                      <div className="relative flex-shrink-0">
                        <div
                          className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center text-xs sm:text-sm font-semibold ${
                            isOwnMessage ? "bg-[#8D2B7E]" : "bg-gray-600"
                          }`}
                        >
          ...
          {/* Error Message */}
          {error && (
            <div className="mx-4 mb-2 p-3 bg-red-900/20 border border-red-500/50 text-red-300 rounded-lg">
              {error}
              <button
            ...
            <form onSubmit={sendMessage} className="flex gap-2 sm:gap-3">
              <div className="flex-1 relative">
                <textarea
                  ...
                  className="w-full p-2 sm:p-3 bg-gray-800/50 border border-gray-700/50 rounded-lg sm:rounded-xl text-white placeholder-gray-400 focus:border-[#8D2B7E] focus:outline-none resize-none text-sm sm:text-base"
                  rows="1"
              ...
              <button
                ...
                className="px-6 py-3 bg-[#8D2B7E] text-white rounded-xl hover:bg-[#8D2B7E]/80 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-2"
              >
                <svg
                  className="w-5 h-5"
                  fill="currentColor"
